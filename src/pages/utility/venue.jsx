...
const Venue = () => {
  ...
  const fetchData = async (pageIndex, pageSize) => {
    try {
      setLoading(true);
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/${user.type}/venue/get-venues`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
        params: {
          page: pageIndex + 1, // Backend is usually 1-indexed
          limit: pageSize,
        },
      });
      const { venues, pagination } = response.data;
      setVendorData(venues);
      setTotal(pagination.totalVenues);
      setTotalPages(pagination.totalPages);
      setHasNextPage(pageIndex + 1 < pagination.totalPages);
    } catch (error) {
      console.log(error);
  ...
  // ... useEffect for data fetch on page or size change ...
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_BASE_URL}/${user.type}/venue/delete-venue/${id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  const handleChangeStatus = async (id, newStatus) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BASE_URL}/${user.type}/venue/change-status/${id}`, {
        method: 'PUT',
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const COLUMNS = [
    ...
    },
    {
      ...
      Cell: ({ cell: { value } }) => (
        value && value?.length > 0 ? (
          <img
            src={value[0]} // Access the first image in the array
            alt="Venue"
    ...
    },
    {
        ...
        Cell: ({ row }) => {
          const user = row.original.address || {};
    ...
    {
      Header: "Sports",
      accessor: "sports", // Just access the array
      Cell: ({ cell: { value } }) => (
        <span>{value?.[0]?.name || "N/A"}</span>
      ),
    },
    ...
    //     Header: "price",
    //     accessor: "price",
    //     Cell: (row) => <span><b>Rs</b>:{row?.cell?.value}</span>,
    //   },
      {
        Header: "numberOfPitches",
      ...
      //   accessor: "timings",
      //   Cell: (row) => {
      //     // Assuming row?.cell?.value is a valid date string
  ...
  if (loading) {
    return <div>Loading Venue...</div>; // Show loading indicator
  }
  return (
    <Card noborder>
      <div className="md:flex pb-6 items-center">
        <h6 className="flex-1 md:mb-0 ">Venue</h6>
