...
const Located = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/${user.type}/location/get-locations`, {
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`${process.env.REACT_APP_BASE_URL}/${user.type}/location/locations/${id}`, {
        headers: {
      ...
      setLoading(false); // Set loading to false when delete is done
    }
  };
  ...
  const actions = [
    {
      ...
      doit: (id) => navigate(`/Located-edit/${id}`)
    },
    {
      name: "delete",
    ...
    <>
      <Card noborder>
        <div className="md:flex pb-6 items-center">
          <h6 className="flex-1 md:mb-0 ">Location</h6>
