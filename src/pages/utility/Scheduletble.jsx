...
const Scheduletble = () => {
  ...
  const fetchData = async (pageIndex, pageSize) => {
    ...
    try {
      ...
      setUserData(response.data.schedule);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_BASE_URL}/${user.type}/schedule/delete-group/${id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  if (loading) {
    return <div>Loading Groups...</div>; // Show loading indicator
  }
    ...
    <>
      <Card noborder>
        <div className="md:flex pb-6 items-center">
          <h6 className="flex-1 md:mb-0 ">Group</h6>
