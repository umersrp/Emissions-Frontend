...
const NotificationTable = () => {
    ...
    const fetchData = async (pageIndex, pageSize) => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/${user.type}/notification/notifications`, {
                headers: {
            ...
            setProductData(response.data.notifications || []);
    ...
    const COLUMNS = [
        ...
        },
        {
            ...
            Cell: ({ cell }) => (
                <span
                    className={`px-2 py-1 rounded text-sm ${cell.value === "scheduled"
        ...
        },
        {
            ...
            accessor: "send_after",
            Cell: ({ row }) => {
                const sendTime = row.original.send_after || row.original.completed_at;
                return (
                    <span>
                        {sendTime ? new Date(sendTime * 1000).toLocaleString() : "N/A"}
                    </span>
                );
            },
        }
    ...
    return (
        <Card noborder>
            <div className="md:flex pb-6 items-center">
                <h6 className="flex-1 md:mb-0 ">Notification</h6>
