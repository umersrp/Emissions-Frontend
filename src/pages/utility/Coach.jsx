...
const CoachPage = () => {
  ...
  const fetchData = async (pageIndex, pageSize) => {
    try {
      setLoading(true)
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/${user.type}/coach/get-coaches`, {
        headers: {
      ...
      setVendorData(response.data.coaches);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_BASE_URL}/${user.type}/coach/coaches/${id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  const handleChangeStatus = async (id, newStatus) => {
    try {
      setLoading(true)
      const response = await fetch(`${process.env.REACT_APP_BASE_URL}/${user.type}/coach/update-status/${id}`, {
        method: 'PUT',
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (response.ok) {
        ...
        console.error('Failed to update Coach status');
  ...
  if (loading) {
    return <div>Loading Coach...</div>; // Show loading indicator
  }
  return (
    <Card noborder>
      <div className="md:flex pb-6 items-center">
        <h6 className="flex-1 md:mb-0 ">Coach</h6>
