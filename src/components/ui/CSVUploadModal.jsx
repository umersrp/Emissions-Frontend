...
const CSVUploadModal = ({
  ...
  const handleDragEnter = useMemo(() => (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (!uploading) {
      setIsDragOver(true);
  ...
  const handleDragLeave = useMemo(() => (e) => {
    e.preventDefault();
    e.stopPropagation();
    setIsDragOver(false);
  }, []);

  const handleDragOver = useMemo(() => (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (!uploading) {
      setIsDragOver(true);
  ...
  const handleDrop = useMemo(() => (e) => {
    ...
    if (droppedFiles && droppedFiles.length > 0) {
      ...
      if (!validExtensions.includes(fileExtension)) {
        alert(`Please upload valid file types: ${validExtensions.join(', ')}`);
        return;
      }
  ...
  }, [uploading, maxSizeMB, onFileSelect]);
          ...
          <div className="flex min-h-full items-center justify-center p-2 sm:p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-[95%] sm:max-w-md md:max-w-lg transform overflow-hidden rounded-xl sm:rounded-2xl bg-white p-4 sm:p-6 text-left align-middle shadow-xl transition-all">
                ...
                {/* File Upload Area - Responsive with drag and drop */}
                <div 
                  className={`mb-2 sm:mb-4 border-2 border-dashed rounded-lg p-3 sm:p-4 md:p-5 text-center transition-all duration-200 ${
                    uploading
                      ? 'border-primary-300 bg-primary-50'
                      : isDragOver
                  ...
                  <div className={uploading ? 'pointer-events-none opacity-70' : ''}>
                    ...
                    {file ? (
                      <div className="space-y-1 sm:space-y-2">
                        ...
                        <p className="text-xs sm:text-sm text-gray-500">
                          {uploading ? 'Upload in progress...' : 'Click to change file or drag & drop'}
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-1 sm:space-y-2">
