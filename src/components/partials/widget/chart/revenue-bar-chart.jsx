...
//   showZeroValues = true,
//   minBarHeight = 5, // Minimum visual height for bars (in percentage of max value)
//   maxBarWidth = null, // Maximum bar width (in percentage, e.g., "60%") - null means use dynamic calculation
//   reduceGap = false, // When true, reduces gap between bars by increasing bar width
// }) => {
//   const [isDark] = useDarkMode();
//   const [isRtl] = useRtl();
...
//   // If reduceGap is true, increase bar width to reduce gaps
//   if (reduceGap) {
//     if (barCount <= 2) columnWidth = "80%";
//     else if (barCount <= 3) columnWidth = "75%";
//     else if (barCount <= 5) columnWidth = "70%";
//     else if (barCount > 15) columnWidth = "40%";
//     else if (barCount > 10) columnWidth = "50%";
//     else columnWidth = "60%";
//   }

//   // If maxBarWidth is provided, use the smaller of the two
//   if (maxBarWidth) {
//     const maxWidthNum = parseInt(maxBarWidth);
//     const currentWidthNum = parseInt(columnWidth);
//     columnWidth = Math.min(maxWidthNum, currentWidthNum) + "%";
//   }
...
//   // Create the color array for the bars
//   const barColors = sortedData.map((item) => {
//     // Make placeholder bars transparent
...
//       toolbar: { show: false },
//       zoom: { enabled: false },
//       offsetX: reduceGap && barCount <= 3 ? -30 : 0,
//       events: {
//         dataPointSelection: (event, chartContext, config) => {
//           // Allow clicks on BOTH series (index 0 for actual bar, index 1 for HoverCap)
//           if (config.seriesIndex !== undefined && config.dataPointIndex !== undefined) {
//             // Ignore clicks on placeholder bars
//             if (sortedData[config.dataPointIndex]?.isPlaceholder) {
//               return;
//             }
//             onBarClick?.(sortedData[config.dataPointIndex]);
//           }
...
//         bottom: 19,
//         left: 0,
//         right: reduceGap ? (barCount <= 3 ? 150 : barCount <= 5 ? 80 : 30) : 0,
//       },
//       show: false
//     },
//     states: {
...
//           // Hide labels for placeholder bars (spaces)
//           if (!value || value.trim() === '') {
//             return '';
//           }
...
//       shared: false,
//       custom: function({ seriesIndex, dataPointIndex, w }) {
//         // Skip tooltip for placeholder bars
//         if (sortedData[dataPointIndex]?.isPlaceholder) {
//           return '';
//         }
