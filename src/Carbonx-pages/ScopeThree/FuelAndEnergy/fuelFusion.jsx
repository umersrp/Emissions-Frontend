...
//                                 onClick={() =>
//                                     navigate(`/fuel-energy-form/${cell.value}`, {
//                                         state: { mode: "view" },
//                                     })
//                                 }
//                             >
...
import useFuelAndEnergyCSVUpload from "@/hooks/scope3/useFuelAndEnergyCSVUpload";
...
const FuelFusion = () => {
    ...
    } = useFuelAndEnergyCSVUpload(buildings);
    ...
    const fetchAllFuelRecords = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BASE_URL}/Fuel-And-Energy/get-All?limit=1000000`,
                {
                    headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
                }
            );
    ...
    const customFormatter = (value, column, row, index) => {
        ...
        if (column.accessor === "didTravelByAir" ||
            column.accessor === "didTravelByTaxi" ||
            column.accessor === "didTravelByBus" ||
            column.accessor === "didTravelByTrain") {
            return value ? "Yes" : "No";
        }
    ...
    const COLUMNS = useMemo(
        () => [
            ...
            { Header: "Remarks", accessor: "remarks", Cell: ({ value }) => capitalizeLabel(value) || "N/A" },
            {
                Header: "Created By",
                accessor: "createdBy.name",
                Cell: ({ cell }) => cell.value || "N/A",
    ...
    return (
        <>
            <Card noborder>
                <div className="md:flex pb-6 items-center">
                    ...
                    <div className="md:flex md:space-x-3 items-center">
                        ...
                        <ExcelExportButton
                            data={records}
                            fetchAllData={fetchAllFuelRecords}
                            columns={COLUMNS}
                            exportFields={[
                                "buildingId.buildingCode",
                                "buildingId.buildingName",
            ...
            <CSVUploadModal
                ...
                title="Bulk Upload Fuel & Energy Records"
                csvState={csvState}
                onFileSelect={handleCSVFileSelect}
                onUpload={handleCSVUpload}
                onReset={resetUpload}
                onDownloadTemplate={downloadFuelAndEnergyTemplate}
