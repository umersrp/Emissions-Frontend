...
const PurchasedGoodServicesListing = () => {
  ...
  const fetchAllPurchasedGoodsRecords = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/Purchased-Goods-Services/get-All?limit=1000000`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
  ...
  const handleDeleteMultiple = async () => {
    ...
    try {
      await Promise.all(
        selectedIds.map(id =>
          axios.delete(`${process.env.REACT_APP_BASE_URL}/Purchased-Goods-Services/delete/${id}`, {
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
          })
        )
      );
  ...
  const COLUMNS = useMemo(
    () => [
      ...
      { Header: "Purchase Category", accessor: "purchaseCategory", Cell: ({ value }) => capitalizeLabel(value) },
  ...
  return (
    <>
          ...
          <div className="md:flex md:space-x-3 items-center">
            ...
            {records.length > 0 && (
              <ExcelExportButton
                ...
                fileName="purchased_goods_current_page"
            ...
            <ExcelExportButton
              data={records}
              fetchAllData={fetchAllPurchasedGoodsRecords}
              columns={COLUMNS}
              exportFields={[
                "buildingId.buildingCode",
                "buildingId.buildingName",
      ...
      <CSVUploadModal
        ...
        title="Bulk Upload Purchased Goods & Services Records"
        csvState={csvState}
