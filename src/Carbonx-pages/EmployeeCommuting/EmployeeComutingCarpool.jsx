...
const EmployeeCommutingFormCarpool = () => {
    ...
    // Track which commute type the user is a passenger in
    ...
    const initializeFormWithOriginalData = (originalData) => {
        ...
        const newFormData = {
            ...
            submittedByEmail: originalData.submittedByEmail || '',
        };

        // Only enable the commute type where user is a passenger
        if (userPassengerDetails.transportType) {
            const transportType = userPassengerDetails.transportType;
            const mode = userPassengerDetails.mode;

            // Enable the appropriate commute type
            if (transportType === 'motorbike') {
                newFormData.commuteByMotorbike = true;
                newFormData.motorbikeMode = mode;
                ...
                // Copy date ranges
                if (originalData.motorbikeDateRange) {
                    newFormData.motorbikeDateRange = originalData.motorbikeDateRange;
                }
                
                if (originalData.motorbikeCarpoolDateRange) {
                    newFormData.motorbikeCarpoolDateRange = originalData.motorbikeCarpoolDateRange;
                }

                // Set the appropriate passenger fields based on whether user is in carry or travel
                if (userPassengerDetails.isDriverCarrying) {
                    ...
                    // Initialize passenger emails array with empty strings (user will fill their distance)
                    newFormData.motorbikePassengerEmails = ['', '', '', '', ''];
                    newFormData.motorbikePassengerUserIds = ['', '', '', '', ''];
                } else if (userPassengerDetails.isTravelWith) {
                    ...
                    newFormData.motorbikeTravelPassengerEmails = ['', '', '', '', ''];
                    newFormData.motorbikeTravelPassengerUserIds = ['', '', '', '', ''];
                }
            }
            else if (transportType === 'car') {
                newFormData.commuteByCar = true;
                newFormData.carMode = mode;
                ...
                // Copy date ranges
                if (originalData.carDateRange) {
                    newFormData.carDateRange = originalData.carDateRange;
                }
                
                if (originalData.carCarpoolDateRange) {
                    newFormData.carCarpoolDateRange = originalData.carCarpoolDateRange;
                }
                ...
                if (userPassengerDetails.isDriverCarrying) {
                    ...
                    newFormData.carPassengerEmails = ['', '', '', '', ''];
                    newFormData.carPassengerUserIds = ['', '', '', '', ''];
                } else if (userPassengerDetails.isTravelWith) {
                    ...
                    newFormData.carTravelPassengerEmails = ['', '', '', '', ''];
                    newFormData.carTravelPassengerUserIds = ['', '', '', '', ''];
                }
            }
            else if (transportType === 'taxi') {
                newFormData.commuteByTaxi = true;
                newFormData.taxiMode = mode;
                ...
                // Copy date ranges
                if (originalData.taxiDateRange) {
                    newFormData.taxiDateRange = originalData.taxiDateRange;
                }
                
                if (originalData.taxiCarpoolDateRange) {
                    newFormData.taxiCarpoolDateRange = originalData.taxiCarpoolDateRange;
                }
                ...
                newFormData.taxiPassengerEmails = ['', '', '', '', ''];
                newFormData.taxiPassengerUserIds = ['', '', '', '', ''];
            }
        }
            ...
            fetchOriginalFormData(currentToken).then((originalData) => {
                ...
                fetchUserData(currentToken).then(() => {
                    ...
                    fetchBuildings(currentToken).then((buildingOptions) => {
                        ...
                        if (originalData) {
                            initializeFormWithOriginalData(originalData);
                        }
                    });
                });
            });
    ...
    const validateForm = () => {
        ...
        // Validate the specific commute where user is a passenger
        if (userPassengerDetails.transportType === 'motorbike') {
        ...
        if (userPassengerDetails.transportType === 'car') {
        ...
        if (userPassengerDetails.transportType === 'taxi') {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Prepare submission data - only include the commute where user is a passenger
            ...
            // Add the commute data based on user's passenger type
            if (userPassengerDetails.transportType === 'motorbike') {
                submissionData.commuteByMotorbike = true;
                submissionData.motorbikeMode = formData.motorbikeMode || 'carpool';
                submissionData.motorbikeDistance = Number(formData.motorbikeDistance) || 0;
                submissionData.motorbikeType = formData.motorbikeType?.value || originalFormData?.motorbikeType || '';
                
                // Convert date range to dates array
                if (formData.motorbikeDateRange) {
                    submissionData.motorbikeDates = dateRangeToDates(formData.motorbikeDateRange).map(date => date.toISOString());
                    submissionData.motorbikeDateRange = formData.motorbikeDateRange;
                }

                // Mark which passenger this user is
                submissionData.passengerIndex = userPassengerDetails.passengerIndex;
                submissionData.isDriverCarrying = userPassengerDetails.isDriverCarrying;
                submissionData.isTravelWith = userPassengerDetails.isTravelWith;
            }

            if (userPassengerDetails.transportType === 'car') {
                submissionData.commuteByCar = true;
                submissionData.carMode = formData.carMode || 'carpool';
                submissionData.carDistance = Number(formData.carDistance) || 0;
                submissionData.carType = formData.carType?.value || originalFormData?.carType || '';
                submissionData.carFuelType = formData.carFuelType?.value || originalFormData?.carFuelType || '';
                
                if (formData.carDateRange) {
                    submissionData.carDates = dateRangeToDates(formData.carDateRange).map(date => date.toISOString());
                    submissionData.carDateRange = formData.carDateRange;
                }
            ...
            if (userPassengerDetails.transportType === 'taxi') {
                submissionData.commuteByTaxi = true;
                submissionData.taxiMode = formData.taxiMode || 'carpool';
                submissionData.taxiDistance = Number(formData.taxiDistance) || 0;
                submissionData.taxiType = formData.taxiType?.value || originalFormData?.taxiType || '';
                
                if (formData.taxiDateRange) {
                    submissionData.taxiDates = dateRangeToDates(formData.taxiDateRange).map(date => date.toISOString());
                    submissionData.taxiDateRange = formData.taxiDateRange;
                }
            ...
            // Submit to backend - you'll need to create this endpoint
            const response = await axios.post(
                `${process.env.REACT_APP_BASE_URL}/employee-commute/carpool-response`,
                submissionData,
                {
                    headers: {
            ...
            if (response.status === 200 || response.status === 201) {
                const userIdToUpdate = userInfo?._id;
                await markUserAsFilled(userIdToUpdate, currentToken);
                
                setSubmitted(true);
                toast.success('Your commute distance submitted successfully!');
    ...
    // Render the relevant commute section based on user's passenger type
            ...
            return (
                <div className="border rounded-lg p-6 bg-white shadow-sm">
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Date range (read-only from original or user select) */}
    ...
    return (
        <div className="max-w-6xl mx-auto p-6">
            <Card title={"Carpool Partner Commute Distance Form"}>
                <div className="mb-8">
                    <div className="text-slate-700 leading-relaxed mb-4 bg-blue-50 rounded-lg border-l-4 border-blue-400 p-4">
                        <p className="text-gray-700">
                            ...
                            Please confirm your portion of the commute distance for the period indicated below.
                        </p>
                    </div>
                    ...
                    {userInfo && (
                        <div className="bg-gray-50 rounded-lg p-4 mb-4">
                            ...
                            <p className="text-sm text-gray-600">Name: {userInfo.name || 'Not provided'}</p>
                ...
                {/* Render only the relevant commute section */}
                ...
                <div className="flex justify-end pt-6 border-t">
                    <Button
                        ...
                        text={loading ? 'Submitting...' : 'Submit Your Distance'}
                        className="btn-dark"
                        disabled={loading || !userPassengerDetails.transportType}
                    />
                </div>
            </Card>
        </div>
